...
$(function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'https://permis-conduire.herokuapp.com/Conducteur',
...
$('#add_Conducteur').on('click', function (e) {
    ...
    if ($Nomfild.val() == "" || $Matriculefild.val() == "" ||  $Telephonefild.val() == "" || $Emailfild.val() == ""||  $Nombre_de_Pointfild.val() == "" ) {
        ...
        $.post('https://permis-conduire.herokuapp.com/Conducteur/Add',
                    data= {
...
$('#login_conductor').on('click', function (e) {
    ...
    if ($Usernamefild.val() == "" || $Passfild.val() == "") {
        ...
        $.post('https://permis-conduire.herokuapp.com/Admins/login',
                    data= {
...
$("#edit").on('click', function () {
   ...
   if ($number.val() == "") {
        ...
        $.post(`https://permis-conduire.herokuapp.com/Conducteur/update/${id}`,
        data= {
...
$('#Check').on('click', function (e) {
    ...
    if ($Matricule.val() == "") {
        ...
        $.ajax({
            type: 'GET',
            url: `https://permis-conduire.herokuapp.com/Conducteur/${Matricule2}`,
            success: function (getContructor) {
                $.each(getContructor, function (i, row) {
            if(row.Matricule != ""){
                $getContructor.append(`
